...
class DeepSeekProviderTest < PrescientTest
  def setup
    super
    @provider = Prescient::Provider::DeepSeek.new(
  ...
  def test_initialize_sets_configuration
    ...
    assert_equal 'deepseek-v4-flash', @provider.options[:chat_model]
    assert_equal 30, @provider.options[:timeout]
  end

  def test_initialize_validates_required_options
    assert_raises(Prescient::Error) do
      Prescient::Provider::DeepSeek.new(chat_model: 'chat')
    end

    assert_raises(Prescient::Error) do
      Prescient::Provider::DeepSeek.new(api_key: 'test')
    end
  end
  ...
  def test_generate_response_success
    ...
    @provider.class.expects(:post).with(
      ...
      has_entries(
        ...
        body:    regexp_matches(/deepseek-v4-flash/),
      ),
    ).returns(response)

    result = @provider.generate_response('test prompt')

    assert_equal 'Hello from DeepSeek', result[:response]
    assert_equal 'deepseek-v4-flash', result[:model]
    assert_equal 'deepseek', result[:provider]
  ...
  def test_health_check_success_and_missing_model
    response = stub(
      success?:        true,
      parsed_response: { 'data' => [{ 'id' => 'deepseek-v4-flash' }] },
    )
  ...
  def test_list_models
    ...
    assert_equal 'deepseek-v4-flash', result.first[:name]
    assert_equal 'deepseek', result.first[:owned_by]
  end
end
