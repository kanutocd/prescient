...
class CoverageGapTest < PrescientTest
  ...
  def test_base_wraps_net_http_errors
    error = assert_raises(Prescient::ConnectionError) do
  ...
  def test_base_extracts_configured_embedding_fields
    provider = Prescient::Base.new(
      context_configs: {
        "article" => {
          fields: %w[title body], format: nil, embedding_fields: %w[title body]
        }
      }
    )
  ...
  def test_base_formats_configured_context_and_falls_back_for_missing_data
    provider = Prescient::Base.new(
      context_configs: {
        "article" => {
          fields: %w[title author], format: "%<title>s by %<author>s", embedding_fields: []
        }
      }
    )
  ...
  def test_base_detects_explicit_and_field_matched_context_types
    provider = Prescient::Base.new(
      context_configs: {
        "article" => { fields: %w[title body], format: nil, embedding_fields: [] },
        "person" => { fields: %w[name email], format: nil, embedding_fields: [] }
      }
    )
