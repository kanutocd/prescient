...
class CLITest < PrescientTest
  ...
  def test_each_command_supports_help
    %w[health generate embed search].each do |command|
  ...
  def test_providers_support_text_and_json_output
    ...
    assert_equal %w[test offline], provider_names
  end
  ...
  def test_config_example_outputs_annotated_schema_backed_yaml
    status, output, errors = run_cli(%w[config example])
  ...
  def test_usage_errors_are_reported_without_a_stack_trace
    ...
    status, _output, errors = run_cli(%w[config unknown])
    ...
    status, _output, errors = run_cli(%w[config validate])
